...
#include "./weights_sd/qparams_qcsnet2_cblk1_input.h"  // brings qcsnet2_input_scale, qcsnet2_input_zero_point
...
static inline ap_int8_c quantize_input_sample(float x_real) {
    ...
    const float inv_s = 1.0f / qcsnet2_cblk1_input_scale;
    float qf = std::nearbyintf(x_real * inv_s) + static_cast<float>(qcsnet2_cblk1_input_zero_point);
...
//     constexpr bool APPLY_MINMAX_NORM = true;  // keep as appropriate

//     int line_no = 1;   // header consumed
//     // int kept    = 0;

//     while (std::getline(file, line)) {
//         ++line_no;

//         // Skip blank lines (log to stdout for visibility)
//         if (line.find_first_not_of(" \t\r\n,") == std::string::npos) {
//             std::cout << "[readCSV] line " << line_no << " blank; skipping.\n";
//             continue;
//         }
...
//         // ---- read floats (skip first CSV column = index) ----
//         bool first_col = true;
//         std::string val;
//         while (std::getline(ls, val, ',')) {
//             if (first_col) { first_col = false; continue; }  // skip index column (set false if no index)
//             val = trim(val);
//             if (val.empty()) continue;                       // treat empty as missing
//             if (col < FIXED_LENGTH1) buf[col++] = std::stof(val);
//         }

//         // ---- Handle short rows exactly as before ----
//         if (col < FIXED_LENGTH1) {
//             if (!PAD_SHORT_ROWS) {
//                 std::cout << "[readCSV] line " << line_no << ": only "
//                           << col << " values (need " << FIXED_LENGTH1
...
//         // ---- Accept the row, then PRINT (guaranteed 1 print per kept row) ----
//         X.push_back(row);
//         y.push_back(label);
//         // ++kept;

//         // std::cout << "[readCSV] kept " << kept
//         //           << " (from line " << line_no << ") int8: ";
//         // for (int j = 0; j < FIXED_LENGTH1; ++j) {
//         //     std::cout << static_cast<int>(row[j])
//         //               << (j + 1 < FIXED_LENGTH1 ? ' ' : '\n');
//         // }
//         // std::cout << std::flush;  // force flush so CSim can’t buffer a line
//     }

//     // std::cout << "[readCSV] done. kept=" << kept << "\n";
//     file.close();
// }
