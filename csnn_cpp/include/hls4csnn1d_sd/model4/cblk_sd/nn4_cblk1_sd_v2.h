...
#pragma HLS STREAM variable=s7 depth=FIXED_LENGTH4
        ...
        hls::stream<ap_int8_c> s8("mp_to_qi_lblk1");       // mp3 -> qi(lblk1)
#pragma HLS STREAM variable=s8 depth=FIXED_LENGTH5         // 24 * 43 = 1032

        hls::stream<ap_int8_c> s9("qi_to_fc_lblk1");       // qi -> fc1
#pragma HLS STREAM variable=s9 depth=FIXED_LENGTH5         // 1032

        hls::stream<ap_int8_c> s10("fc_to_lif_lblk1");     // fc1 -> lif(lblk1)
#pragma HLS STREAM variable=s10 depth=FIXED_LENGTH6        // 64

        hls::stream<ap_int8_c> s11("lif_to_qi_lblk2");     // lif -> qi(head)
#pragma HLS STREAM variable=s11 depth=FIXED_LENGTH6        // 64

        hls::stream<ap_int8_c> s12("qi_to_fc_lblk2");      // qi -> fc(head)
#pragma HLS STREAM variable=s12 depth=FIXED_LENGTH6        // 64

        hls::stream<ap_int8_c> s13("fc_to_lif_lblk2");     // fc -> lif(head)
#pragma HLS STREAM variable=s13 depth=FIXED_LENGTH7        // 4
...
#pragma HLS PIPELINE II=1
            ...
            //---------------------------------------------------------------------------
            // quant identity before first linear block (24 × 43 → 24 × 43)
            qcsnet4_lblk1_qi_int.forward(
                s8, s9,
                qcsnet4_lblk1_input_act_scale_int);

            // dense head block 1: linear(1032→64) → lif(64)
            ...
            //---------------------------------------------------------------------------
            // quant identity before final linear head (64 → 64)
            qcsnet4_lblk2_qi_int.forward(
                s11, s12,
                qcsnet4_lblk2_input_act_scale_int);

            // dense head block 2: linear(64→4) → lif(4)
...
private:
    ...
    Conv1D_SD<16, 24, 3, 1, CONV_IN_LENGTH2>                  qcsnet4_cblk2_qconv1d_int;
    ...
    /* dense head block 1: 24×43 → 64 → 64 */
    QuantIdentityPerTensor_Int8<OUT_CH2, POOL2_OUT_LEN>       qcsnet4_lblk1_qi_int;      // 24×43 = 1032
    Linear1D_SD<LINEAR_IN_SIZE1, LINEAR_OUT_SIZE1>            qcsnet4_lblk1_qlinear1d_int; // 1032→64
    LIF1D_SD_Integer<LINEAR_OUT_SIZE1, 1>                     qcsnet4_lblk1_lif1d_int;     // 64×1 = 64

    /* dense head block 2: 64 → 4 */
    QuantIdentityPerTensor_Int8<LINEAR_OUT_SIZE1, 1>          qcsnet4_lblk2_qi_int;        // 64×1 = 64
    Linear1D_SD<LINEAR_OUT_SIZE1, 4>                          qcsnet4_lblk2_qlinear1d_int; // 64→4
